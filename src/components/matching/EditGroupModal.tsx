...
import { X, Search, Check, Shirt, Trash2 } from 'lucide-react';
import { Button } from '@/components/common/Button';
import { scaleIn } from '@/lib/animations';
import { LAYERS } from '@/types';
import type { ClothingItem, MatchingGroup } from '@/types';
...
interface EditGroupModalProps {
  ...
  items: ClothingItem[];
  onClose: () => void;
  onSave: (id: string, updates: { name?: string; item_ids?: string[] }) => void;
  onDelete: (id: string) => void;
}

export function EditGroupModal({ open, group, items, onClose, onSave, onDelete }: EditGroupModalProps) {
  ...
  // Populate when group changes
  useEffect(() => {
    if (group) {
      setName(group.name);
      setSelectedIds(new Set(group.item_ids));
    }
  }, [group]);
  ...
  const handleSave = () => {
    if (!group || !name.trim() || selectedIds.size === 0) return;
    onSave(group.id, { name: name.trim(), item_ids: Array.from(selectedIds) });
    handleClose();
  };
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between p-5 border-b border-parchment-deep shrink-0">
              <h2 className="font-display text-xl text-ink">Edit Group</h2>
            ...
            <div className="flex items-center justify-between p-5 border-t border-parchment-deep shrink-0">
              <Button
                ...
                icon={<Trash2 size={14} />}
                className={confirmDelete ? 'text-rouge hover:text-rouge' : ''}
              >
                {confirmDelete ? 'Confirm Delete' : 'Delete'}
              </Button>
              <div className="flex gap-2">
                <span className="text-xs text-ink-muted self-center">{selectedIds.size} selected</span>
